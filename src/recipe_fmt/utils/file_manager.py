...
class FileManager:
    ...
    def get_corresponding_yaml_path(self, markdown_path: Path) -> Path:
        ...
        return Path(yaml_dir / yaml_filename)

    def get_corresponding_pdf_path(self, yaml_path: Path, category: str | None = None) -> Path:
        ...
        return Path(pdf_dir / pdf_filename)

    def get_stale_pipeline_files(self) -> dict[str, list[Path]]:
        """Get stale files that need processing in the pipeline.

        Returns:
            Dictionary with keys 'markdown_to_yaml' and 'yaml_to_pdf'
            containing lists of files that are stale and need processing
        """
        stale_files: dict[str, list[Path]] = {"markdown_to_yaml": [], "yaml_to_pdf": []}

        try:
            # STEP_8: Check for stale markdown to YAML conversions
            markdown_files = self.get_recipe_files()
            ...
            # STEP_9: Get all YAML files for PDF processing (for now, all YAML files)
            yaml_dir = Path(self.cfg_dict["recipe_dir"]) / self.cfg_dict["yaml_subdir"]
            if yaml_dir.exists():
                yaml_files = list(yaml_dir.glob("*.yaml"))
                stale_files["yaml_to_pdf"] = yaml_files
                self.logger.debug("Found %d YAML files for processing", len(yaml_files))
            ...
            error_msg = f"Error getting pipeline files: {e}"
            self.logger.exception(error_msg)
            return stale_files
    ...
    def clean_generated_files(self, file_types: list[str] | None = None) -> int:
