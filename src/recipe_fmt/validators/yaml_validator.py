...
class YAMLValidator:
    ...
    def __init__(self, cfg_dict: dict = None) -> None:
        ...
        self.logger.info(
            "YAMLValidator initialized with strict mode: %s",
    ...
    def validate_yaml_file(self, file_path: str | Path) -> ValidationResult:
                ...
                for ingredient in yaml_data["ingredients"]:
                    if isinstance(ingredient, dict):
                        ...
                        if ingredient.get("amount") is None:
                            ...
                            if any(
                                word in ingredient_name for word in ["garnish", "for serving", "optional", "to taste"]
                            ):
                                ingredient["amount"] = 0.25
                                ingredient["unit"] = "portion"
                                self.logger.debug(
                                    "Fixed null amount for garnish ingredient in %s: %s",
                                    source_name,
                                ...
                                self.logger.warning(
                                    "Fixed null amount for ingredient in %s: %s",
                                    source_name,
                                    ingredient.get("ingredient"),
                                )
        ...
        try:
            ...
            try:
                ...
                if self.cfg_dict.get("strict_validation", True):
                    return ValidationResult(valid=False, yaml_data=yaml_data, errors=errors, warnings=warnings)
            ...
            if is_valid:
                self.logger.info(
                    "Validation successful for %s: %s (%d warnings)",
                    source_name,
                    recipe.title if recipe else "unknown",
                    len(warnings),
                )
            else:
                self.logger.warning(
            ...
            return ValidationResult(
                valid=is_valid, recipe=recipe, yaml_data=yaml_data, errors=errors, warnings=warnings
            )
    ...
    def _validate_business_rules(
        self, yaml_data: dict, errors: list[ValidationError], warnings: list[ValidationError]
    ) -> None:
    ...
    def _validate_weight_consistency(self, recipe: Recipe, warnings: list[ValidationError]) -> None:
        ...
        if not self.cfg_dict.get("check_weights", True):
            return
    ...
    def _estimate_ingredient_weight(self, ingredient: Ingredient) -> int | None:
        ...
        # STEP_22: Look up weight per unit (case-insensitive unit lookup)
        unit_weights_dict = None
        for unit_key in self.unit_weights:
            if unit.lower() == unit_key.lower():
                unit_weights_dict = self.unit_weights[unit_key]
                break

        if unit_weights_dict:
            # Try exact match first
            if ingredient_name in unit_weights_dict:
                return int(ingredient.amount * unit_weights_dict[ingredient_name])

            # Try partial matches
            for key, weight_per_unit in unit_weights_dict.items():
    ...
    def _validate_nutrition_data(self, recipe: Recipe, warnings: list[ValidationError]) -> None:
        ...
        """
        # Skip validation if disabled in configuration
        if not self.cfg_dict.get("check_nutrition", False):
            return
    ...
    def get_validation_stats(self) -> dict:
        ...
        return {
            ...
            "supported_categories": sorted(self.valid_categories),
