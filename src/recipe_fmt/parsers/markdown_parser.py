...
class MarkdownParser:
    ...
    def parse_recipe_file(self, file_path: str | Path) -> ParseResult:
    ...
    def _parse_markdown_content(self, markdown_content: str, recipe_name: str) -> ParseResult:
        ...
        try:
            ...
            self.logger.debug(
                "OpenAI parsing successful for %s (%d tokens, $%.4f)",
                recipe_name,
    ...
    def _clean_openai_response(self, yaml_data: dict, recipe_name: str) -> dict:
        ...
        try:
            ...
            if "ingredients" in yaml_data and isinstance(yaml_data["ingredients"], list):
                for _i, ingredient in enumerate(yaml_data["ingredients"]):
    ...
    def _validate_and_parse_yaml(
        ...
        try:
            ...
            try:
                ...
                self.logger.info(
                    "Successfully parsed recipe: %s (category: %s, %d ingredients)",
                    recipe.title,
                    recipe.category,
                ...
                self.logger.debug(
                    "Weight conversion coverage: %d/%d ingredients have weights",
