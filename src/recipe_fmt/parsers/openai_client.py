...
class OpenAIClient:
    ...
    def _get_cached_response(self, cache_key: str) -> OpenAIResponse | None:
    ...
    def _make_openai_request(self, messages: list[dict], attempt: int = 1) -> OpenAIResponse:
        ...
        try:
            ...
            self.logger.info(
                "OpenAI request successful - tokens: %s, cost: $%.4f",
                tokens_used,
                cost_estimate or 0,
            )
...
"""
        ...
        try:
            ...
            for attempt in range(1, max_retries + 1):
                ...
                if attempt < max_retries:
                    ...
                    self.logger.warning(
                        "Request failed, retrying in %.1f seconds (attempt %d/%d)",
                        delay,
                        attempt,
                        max_retries,
                    )
    ...
    def _get_recipe_parsing_prompt(self) -> str:
        ...
        """
        return """You are an expert recipe parsing assistant specializing in converting
markdown recipes into structured YAML format for an AI-powered recipe card system.

## TASK
Convert the provided markdown recipe into valid YAML following the exact schema below.
Pay careful attention to data types, unit standardization, and weight estimation.

## OUTPUT SCHEMA
```yaml
title: string (recipe name, max 100 characters)
category: string (must be one of: Meat, Side, Main, Soup, Sauce, Breakfast, Salad,
  Baking, Dessert, Other)
