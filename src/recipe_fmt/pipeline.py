...
This module coordinates the entire recipe pipeline: markdown → YAML → PDF.
It uses intelligent staleness detection to only process files that need updating.
...
class RecipePipeline:
    ...
    def run(self, force_rebuild: bool = False, demo_mode: bool = False) -> dict:
        ...
        try:
            self.logger.info("Starting recipe pipeline (force_rebuild=%s, demo=%s)", 
            ...
            self.logger.info("Pipeline completed: %d files processed, %d errors", 
                           results["total_processed"], len(results["errors"]))
            
            if metrics["total_tokens_used"] > 0:
                self.logger.info("OpenAI usage: %d tokens, $%.4f, %d cached responses", 
                               metrics["total_tokens_used"], 
                               metrics["total_cost_estimate"],
                               metrics["cached_responses"])
            
            if metrics["total_pdf_pages"] > 0:
                self.logger.info("PDF generation: %d pages, %.1f MB total", 
    ...
    def _process_markdown_to_yaml(self, md_file: Path) -> dict:
        ...
        try:
            ...
            if parse_result.yaml_content:
                validation_result = validator.validate_yaml_content(
                    parse_result.yaml_content, 
                    f"parsed_{md_file.name}"
                )
                ...
                if not validation_result.valid:
                    ...
                    # Save invalid YAML for debugging if configured
                    if self.config.debug:
                        debug_file = yaml_file.with_suffix('.yaml.debug')
                        debug_file.write_text(parse_result.yaml_content)
                        self.logger.warning("Saved invalid YAML for debugging: %s", debug_file)
                    
                    return result
                
                # Log validation warnings
                for warning in validation_result.warnings:
                    self.logger.warning("YAML validation warning for %s [%s]: %s", 
                                      md_file.name, warning.field, warning.message)
            ...
            if parse_result.tokens_used:
                self.logger.info("OpenAI usage: %d tokens, $%.4f", 
                               parse_result.tokens_used, parse_result.cost_estimate or 0)
    ...
    def _process_yaml_to_pdf(self, yaml_file: Path) -> dict:
        ...
        try:
            ...
            for warning in validation_result.warnings:
                self.logger.warning("YAML validation warning for %s [%s]: %s", 
                                  yaml_file.name, warning.field, warning.message)
            ...
            self.logger.info("PDF stats: %d pages, %d bytes", 
...
def setup_logging(log_level: str = "WARNING") -> None:
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler("recipe_pipeline.log", mode="a")
        ]
    )
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Recipe Binder - AI-Powered Recipe Card Pipeline"
    )
    
    parser.add_argument(
        "--force", "-f",
        action="store_true",
        help="Force rebuild all files regardless of staleness"
    )
    
    parser.add_argument(
        "--demo", "-d", 
        action="store_true",
        help="Run in demo mode with sample recipes"
    )
    
    parser.add_argument(
        "--clean", "-c",
        action="store_true",
        help="Clean generated PDF files"
    )
