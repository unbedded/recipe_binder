...
class TemplateLoader:
    ...
    def __init__(self, cfg_dict: dict = None) -> None:
        ...
        self.logger.info("TemplateLoader initialized with %d search paths", 
                        len(self.engine.template_paths))
        ...
        try:
            ...
            if self.cfg_dict.get("fallback_to_builtin", True) and not template_name.endswith(".yaml"):
                ...
                if DefaultTemplates.is_default(fallback_name):
                    self.logger.warning("Template not found: %s, trying default: %s", 
                                      template_name, fallback_name)
                    return self.load_template(fallback_name)
            
            raise e
    
    def load_from_file(self, file_path: Union[str, Path]) -> CardTemplate:
    ...
    def _create_minimal_template(self) -> CardTemplate:
        ...
        template = CardTemplate(
            ...
            version="1.0",
            description="Auto-generated minimal template"
        )
...
def create_loader(template_paths: List[Union[str, Path]] = None) -> TemplateLoader:
...
if __name__ == "__main__":
    ...
    print(f"Template Loader Statistics:")
