...
class CardTemplate:
    ...
    # Typography styles
    title_style: TypographyStyle = field(default_factory=lambda: TypographyStyle(
        font_name="Helvetica-Bold", font_size=18, alignment="center", space_after=6
    ))
    category_banner_style: TypographyStyle = field(default_factory=lambda: TypographyStyle(
        font_name="Helvetica-Bold", font_size=12, alignment="center", color="white"
    ))
    section_header_style: TypographyStyle = field(default_factory=lambda: TypographyStyle(
        font_name="Helvetica-Bold", font_size=14, space_before=8, space_after=4
    ))
    metadata_style: TypographyStyle = field(default_factory=lambda: TypographyStyle(
        font_size=9, alignment="center", color="#666666", space_after=4
    ))
    description_style: TypographyStyle = field(default_factory=lambda: TypographyStyle(
        font_name="Helvetica-Oblique", font_size=10, alignment="center", space_after=6
    ))
    ingredient_style: TypographyStyle = field(default_factory=lambda: TypographyStyle(
        font_size=9, space_after=2
    ))
    instruction_style: TypographyStyle = field(default_factory=lambda: TypographyStyle(
        font_size=10, alignment="justify", space_after=3
    ))
    notes_style: TypographyStyle = field(default_factory=lambda: TypographyStyle(
...
class TemplateEngine:
    ...
    def __init__(self, cfg_dict: dict = None) -> None:
        ...
        self.logger.info("TemplateEngine initialized with %d search paths", 
                        len(self.template_paths))
        ...
        try:
            ...
            try:
                ...
                if not isinstance(template_data, dict):
                    raise TemplateError(f"Invalid template format: expected dictionary at root")
            ...
            self.logger.info("Template loaded successfully: %s (%s)", 
    ...
    def _parse_template_data(self, data: dict, source_path: Path) -> CardTemplate:
        ...
        try:
            ...
            template.title_style = self._parse_typography_style(
                typography_data.get('title', {}), template.title_style
            )
            ...
            template.notes_style = self._parse_typography_style(
                typography_data.get('notes', {}), template.notes_style
            )
            ...
            self.logger.debug("Parsed template: %s with %d typography styles", 
                           template.name, 8)
    ...
    def _validate_template(self, template: CardTemplate) -> None:
        ...
        styles = [
            template.title_style, template.category_banner_style,
            template.section_header_style, template.metadata_style,
            template.description_style, template.ingredient_style,
            template.instruction_style, template.notes_style
        ]
    ...
    def create_generator(self, template: CardTemplate, **overrides) -> 'PDFCardGenerator':
        ...
        """
        from ..generators.pdf_generator import PDFCardGenerator, CardLayout
        from ..models.config import DisplayConfig
        ...
        display_config = DisplayConfig(
            ...
            font_family=overrides.get('font_family', 'Helvetica'),
            color_scheme=overrides.get('color_scheme', 'default')
        )
...
def create_engine(template_paths: List[Union[str, Path]] = None) -> TemplateEngine:
