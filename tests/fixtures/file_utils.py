...
from .openai_responses import ALL_MOCK_RESPONSES
...
class MockFileSystem:
    ...
    def __init__(self, base_path: Optional[Path] = None):
        ...
        # Create directories
        for directory in [self.markdown_dir, self.yaml_dir, self.pdf_dir, 
                         self.templates_dir, self.config_dir]:
    ...
    def create_file_with_timestamp(self, file_path: Union[str, Path], 
                                  content: str, timestamp: float) -> Path:
    ...
    def create_staleness_test_files(self) -> Dict[str, Dict[str, Path]]:
        ...
        target1 = self.create_file_with_timestamp(
            self.pdf_dir / "fresh_recipe.pdf",
            "PDF content",
            current_time
        )
        ...
        target2 = self.create_file_with_timestamp(
            self.pdf_dir / "stale_recipe.pdf",
            "Old PDF content",
            old_time
        )
    ...
    def create_error_scenario_files(self) -> Dict[str, Path]:
        ...
        large_content = "# Large Recipe\n" + "## Ingredients\n" + "\n".join([
            f"- ingredient_{i}" for i in range(10000)
        ]) + "\n## Instructions\n" + "\n".join([
            f"{i}. Step {i}" for i in range(1, 5001)
        ])
...
@contextmanager
def temporary_template_files(templates: Dict[str, Dict[str, Any]]) -> ContextManager[Dict[str, Path]]:
...
def cleanup_temp_files(file_paths: Union[List[Path], Dict[str, Path]]) -> None:
