...
from .file_utils import FileSystemTestHelper, MockFileSystem
...
def sample_template_files(mock_file_system):
    ...
    valid_templates = {name: data for name, data in ALL_TEMPLATE_DATA.items() if name in get_valid_template_names()}
...
def mock_openai_error_responses():
    ...
    with patch("openai.OpenAI") as mock_client_class:
        ...
        error_responses = create_retry_sequence_responses(failure_count=2, final_success=True)
...
def pdf_generator(test_config, mock_reportlab):
    ...
    display_config = DisplayConfig(
        show_weights=True, show_purpose=True, font_family="Helvetica", color_scheme="default"
    )
...
@pytest.fixture(params=[("valid_small", 10), ("valid_medium", 100), ("valid_large", 1000)])
...
@pytest.fixture(params=[("8.5x4", 8.5, 4.0), ("6x4", 6.0, 4.0), ("5x3", 5.0, 3.0), ("letter", 8.5, 11.0)])
...
def cleanup_test_artifacts():
    ...
    test_patterns = ["test_*.pdf", "test_*.yaml", "test_*.md", "*_test_*"]
