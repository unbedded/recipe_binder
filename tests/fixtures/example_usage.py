...
from tests.fixtures import (
    ...
    MockFileSystem, FileSystemTestHelper,
    create_temp_recipe_files, cleanup_temp_files
)

from tests.fixtures.sample_recipes import (
    get_sample_recipe, get_sample_markdown, 
    create_test_recipe_variations
)

from tests.fixtures.template_data import (
    get_template_data, create_template_variations,
    get_valid_template_names, get_invalid_template_names
)

from tests.fixtures.openai_responses import (
    create_mock_openai_response, create_retry_sequence_responses,
    create_markdown_input_samples, get_mock_response_by_content_type
)
...
def demonstrate_sample_recipes():
    ...
    categories = set(recipe.category for recipe in variations)
...
def demonstrate_openai_responses():
    ...
    custom_response = create_mock_openai_response(
        "Custom test content", tokens_used=150
    )
    ...
    retry_responses = create_retry_sequence_responses(
        failure_count=3, final_success=True
    )
...
def demonstrate_integration_scenarios():
    ...
    print(f"   Output: PDF card (8.5x4 inch)")
