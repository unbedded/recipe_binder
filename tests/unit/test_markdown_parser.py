...
"""
        ...
        result = parser._validate_and_parse_yaml(
            valid_yaml, "test.md", 100, 0.001, False
        )
...
"""
        ...
        parser = MarkdownParser(self.config, {"validate_yaml": True})
        result = parser._validate_and_parse_yaml(
            invalid_yaml, "test.md", 100, 0.001, False
        )
...
"""
        ...
        parser = MarkdownParser(self.config, {"validate_yaml": True})
        result = parser._validate_and_parse_yaml(
            invalid_recipe_yaml, "test.md", 100, 0.001, False
        )
    ...
    def test_yaml_validation_disabled(self):
        ...
        result = parser._validate_and_parse_yaml(
            yaml_content, "test.md", 100, 0.001, False
        )
        ...
        def worker(worker_id):
            try:
                ...
                with patch.object(parser, '_parse_markdown_content') as mock_parse:
                    mock_parse.return_value = ParseResult(
                        success=True, 
                        yaml_content=f"title: Recipe {worker_id}"
                    )
